// Couvre la validation WCAG : la palette LCh recalculée doit honorer les
// contraintes de contraste déclarées dans `mapping.colors.<family>.wcag`.

import test from 'node:test';
import assert from 'node:assert/strict';
import { validateWcag } from './wcag.js';

// Sans bloc `wcag` dans le mapping, aucun check n'est émis (et zéro échec).
...
// Le mapping ADEME Blue/Red reproduit les ratios documentés en Phase 1 (zéro fail)
// et les tokens reflètent bien le rename (`blue-ate-*`, `red-laura-*`).
...
// Un grade trop clair contre fond blanc échoue avec `ok=false`.
...
// Pointer un grade absent de la palette → `ok=false` avec `missing: true`,
// distinct d'un échec de ratio.
